...
class BaseFeatureExtractor(nn.Module):
    def __init__(self, 
        ...
        if len(self.trainable_part) > 0:
            if not isinstance(self.trainable_part, nn.Sequential):
                self.trainable_part = nn.Sequential(OrderedDict(self.trainable_part))
            self.trainable_part = self.trainable_part.to(device, non_blocking=True)
        else:
...
class MLPCosine(BaseFeatureExtractor):
    def __init__(self, 
                 ...
                 device: str = "cpu",
                 **kwargs):
        super(MLPCosine, self).__init__(model_name=model_name, state_dict=state_dict, trainable_from_layer=trainable_from_layer, device=device)
    ...
    def load_model_safely(model, checkpoint_path, device="cpu"):
        """
        Carica automaticamente un checkpoint gestendo i casi:
        - trainable_from_layer = None / 'all' / specifica
        - state_dict salvato con 'freezed_part' o 'trainable_part'
        - state_dict = None (non carica nulla)
        """
        ...
        # Caso 1: perfettamente compatibili
        ...
        # Caso 3: checkpoint completo (senza prefissi)
